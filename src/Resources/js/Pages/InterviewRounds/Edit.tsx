...
import { EditInterviewRoundProps, EditInterviewRoundFormData } from './types';
import { usePage } from '@inertiajs/react';
import { useEffect, useState } from 'react';
import axios from 'axios';

export default function EditInterviewRound({ interviewround, onSuccess }: EditInterviewRoundProps) {
    const { jobpostings } = usePage<any>().props;

    const { t } = useTranslation();
    const { data, setData, put, processing, errors } = useForm<EditInterviewRoundFormData>({
        name: interviewround.name ?? '',
        sequence_number: interviewround.sequence_number ?? '',
        description: interviewround.description ?? '',
        status: interviewround.status?.toString() ?? '0',
        job_id: interviewround.job_id?.toString() || '',
    });

    const submit = (e: React.FormEvent) => {
        e.preventDefault();
        put(route('recruitment.interview-rounds.update', interviewround.id), {
            onSuccess: () => {
                onSuccess();
            }
        });
    ...
    return (
        <DialogContent>
            <DialogHeader>
                <DialogTitle>{t('Edit Interview Round')}</DialogTitle>
            </DialogHeader>
