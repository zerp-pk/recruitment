...
export default function EditCandidate({ candidate, onSuccess }: EditCandidateProps) {
    ...
    const { data, setData, put, processing, errors } = useForm<any>(getInitialFormData());
    ...
    return (
        <DialogContent className="max-w-4xl">
            <DialogHeader>
                <DialogTitle>{t('Edit Candidate')}</DialogTitle>
            </DialogHeader>
                ...
                {(jobPostingSettings.visibility.includes('resume') || jobPostingSettings.visibility.includes('cover_letter')) && (
                    <div className="grid grid-cols-2 gap-4">
                        {jobPostingSettings.visibility.includes('resume') && (
                            <div>
                                <Label htmlFor="resume_path">{t('Resume/CV')}</Label>
                                <Input
                                    id="resume_path"
                                    type="file"
                                ...
                                {candidate.resume_path && !data.resume && (
                                    <div className="mt-2 p-2 border rounded-lg bg-blue-50">
                                        <div className="flex items-center space-x-2">
                                            <FileText className="h-8 w-8 text-blue-500" />
                                            <span className="text-sm text-blue-700">{t('Current resume uploaded')}</span>
                                        </div>
                                    </div>
                                )}
                            </div>
                        ...
                        {jobPostingSettings.visibility.includes('cover_letter') && (
                            <div>
                                <Label htmlFor="cover_letter_path">{t('Cover Letter')}</Label>
                                <Input
                                ...
                                {candidate.cover_letter_path && !data.cover_letter && (
                                    <div className="mt-2 p-2 border rounded-lg bg-blue-50">
                                        <div className="flex items-center space-x-2">
                                            <FileText className="h-8 w-8 text-blue-500" />
                                            <span className="text-sm text-blue-700">{t('Current cover letter uploaded')}</span>
                                        </div>
                                    </div>
                                )}
                            </div>
